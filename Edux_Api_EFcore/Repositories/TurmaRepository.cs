...
namespace Edux_Api_EFcore.Repositories
{
    public class TurmaRepository : ITurmaRepository
    {
       ...
       public TurmaRepository()
        ...
        public void Editar(Turma turma, List<ProfessorTurma> professores, List<AlunoTurma> alunos)
        {
            try
            {
                if (BuscarPorId(turma.Id) == null)
                    throw new Exception("Impossível alterar Turma pois faltam dados.");

                foreach (ProfessorTurma item in professores)
                {
                    turma.ProfessoresTurmas.Add(new ProfessorTurma
                    {
                        Descricao = item.Descricao,
                        IdUsuario = item.IdUsuario,
                        IdTurma = turma.Id
                ...
                foreach (AlunoTurma item in alunos)
                {
                    turma.AlunosTurmas.Add(new AlunoTurma
                    {
                        Matricula = item.Matricula,
                        IdUsuario = item.IdUsuario,
                        IdTurma = turma.Id
